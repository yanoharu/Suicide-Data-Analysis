...
display_statistics_table <- function(){
  ...
  # 一時ファイルを読み込む
  htmltools::includeHTML(temp_file)
}
...
I pick up only results where I can find 5% significance in suicide report dummy, which are 20s,30s,50s of men and 20s,40s,50s and 60s of women' results. Although I could find correlation between suicide and suicide news, we must be careful to interpret the results. Looking at diagnostic test in 4.3, you can find model of "wom_20s" doesn`t pass normality test, model of "man_50s" doesn't pass serial correlation test, which means there's room for improvement in model specification. Moreover, coefficient of suicide_report1 in "man_50s", suicide_report3 in "wom_40s" are negative with significance and it's hard to explain these results.
```{r, echo = FALSE, results="asis"}
...
for (file_name in file_name){
  ...
  # 一時ファイルを読み込む
  table_output <- htmltools::includeHTML(temp_file)
  print(table_output)}  
}
...
### 4.3 Diagnostic Test
In time series empirical model, we should validate results. 
```{r, echo = FALSE, results="asis"}
...
display_diagnostic_test(gender_age)
```
...
```{r, echo = FALSE, results="asis"}
display_diagnostic_test <- function(file_names){
  # Create a div with flex display
  div_style <- "display: flex; flex-wrap: wrap; justify-content: space-between;"
  div_content <- lapply(file_names, function(file_name) {
    gender <- stringr::str_to_lower(substr(file_name, 1, 3))
    age <- substr(file_name, 4, 6)
    ...
    # 一時ファイルを読み込む
    table_output <- htmltools::includeHTML(temp_file)
    htmltools::tags$div(style = "flex-basis: 30%;", table_output)  # Change this line
  })
  print(htmltools::tags$div(style = div_style, div_content))
}
